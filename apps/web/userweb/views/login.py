...
def login_handle(request):
    ...
    try:
        ...
        if id_token:
            ...
            user_department = sso_user_profile['user_profile_department']
            user_email = sso_user_profile['email']
            user_name = sso_user_profile['name'].split('(')[0].strip()

        # Query user according loginid from DB
        obj_user = Account.objects.get(loginid=loginid)
        ...
        # Update user profile
        obj_user.last_login = arrow.now().timestamp()
        obj_user.department = user_department
        obj_user.email = user_email
        obj_user.name = user_name
        obj_user.save()
        ...
        res_data = {
            ...
            'create_time': arrow.get(obj_user.create_time).to(obj_user.timezone).format('YYYY-MM-DD HH:mm:ss'), 'timezone': obj_user.timezone,
            'last_login': arrow.get(obj_user.last_login).to(obj_user.timezone).format('YYYY-MM-DD HH:mm:ss'), 'last_loginid': loginid, 'name_abb': name_abb
        }

        # Add edit time
        if obj_user.edit_time:
            res_data['edit_time'] = arrow.get(obj_user.edit_time).to(obj_user.timezone).format('YYYY-MM-DD HH:mm:ss')
        else:
